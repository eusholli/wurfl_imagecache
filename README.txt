wurfl_ImageCache is a dynamic image manipulation and cache tool. It allows you to
...
until the namespace or the entire wurfl_imagecache is flushed.

Getting Started:

1. Upload and enable both the wurfl_ImageCache and wurfl_ImageCache UI modules.

2. Go to Administer -> Site Building -> wurfl_ImageCache. Click on the local task tab
...
4. Add actions to your preset that tell wurfl_ImageCache how to manipulate the
...
http://example.com/files/wurfl_imagecache/preset-name/files/image-name.jpg
...
http://example.com/files/wurfl_imagecache/product_thumbnail/files/green-widget...
...
wurfl_ImageCache also defines a theme function that you can use in your modules and
...
<?php
print theme('wurfl_imagecache', 'preset_namespace', $image_filepath, $alt, $title, $attributes);
?>

Change 'preset_namespace' to the name of your wurfl_imagecache preset and make sure
...
Using wurfl_ImageCache with Contributed Modules:

wurfl_ImageCache presets can be put to use in various other modules. For example, when
...
wurfl_ImageCache, but the concepts are the same.)
